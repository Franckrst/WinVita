...
#include <stdint.h>
#include "debug.h"          // DYN86_G2H (fastmmu) used by Push()/PushExit() below
#include "regs.h"
...
{
    R_ESP -= 4;
    *((uint32_t*)DYN86_G2H(R_ESP)) = v;   /* fastmmu: guest ESP, host deref */
}
#endif
...
{
    R_ESP -= 4;
    *((uint32_t*)DYN86_G2H(R_ESP)) = my_context->exit_bridge;
}
