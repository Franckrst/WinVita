...
{
    // fastmmu: `p` is the HOST effective address (guest VA + membase folded at
    // translation), but getDB/cleanDBFromAddressRange are keyed by GUEST VA.
    // Subtract membase so a CLFLUSH-driven SMC invalidation hits the right block
    // on hardware (arena mode), not just under identity mapping (qemu). No-op
    // subtract when membase==0. (audit D7)
    cleanDBFromAddressRange((uintptr_t)p - dyn86_membase, 8, 0);
}
...
{
    // D2Vita: no native-call inlining in the PoC
    (void)dyn; (void)addr; (void)calladdress; (void)retn;
    return 0;
}
static __attribute__((unused)) int isNativeCall_unused(dynarec_arm_t* dyn, uintptr_t addr, uintptr_t* calladdress, int* retn)
{
