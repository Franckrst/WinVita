...
typedef struct dynarec_arm_s {
    ...
    int                 budget_tail;// D2Vita (D2_BUDGETTAIL=1): offset, dans le
                                    // code ARM du bloc, du chemin d'expiration
                                    // du budget pose en QUEUE. Ecrit a la fin de
                                    // chaque passe, relu au PROLOGUE de la passe
                                    // suivante — c'est le mecanisme de « marque »
                                    // deja employe par MARK/GETMARK pour les
                                    // branchements AVANT (pass2 mesure, pass3
                                    // emet ; l'ecart de taille pass2/pass3 est
                                    // deja detecte par dynarec_arm.c).
    int                 memintrin_mark;// D2Vita (D2_MEMINTRIN) : meme mecanisme
                                    // de marque que budget_tail, pour la branche
                                    // « le natif a refuse » du prologue
    ...
    //
    // D2_MMUFOLD — dialogue entre les macros GETED* et geted() au sujet du
    // PLIAGE de la base fastmmu dans la constante d'un adressage ABSOLU.
    // mmufold_req  : pose par la macro JUSTE avant l'appel a geted(), et
    //                consomme (remis a 0) par geted a son entree. Un site qui
    //                ne le pose pas ne peut donc jamais declencher le pliage —
    //                c'est ce qui rend les ~270 appels directs a geted() dans
    //                les fichiers d'opcodes surs SANS les auditer un par un.
    // mmufold_done : reponse de geted. Vrai => l'adresse rendue est DEJA une
    //                adresse hote, MMU_ADDR ne doit PAS etre reapplique (la
    //                double traduction est le piege documente en tete de
    //                GETEDO/GETEDO2 dans dynarec_arm_helper.h).
    int                 mmufold_req;
    int                 mmufold_done;
    // D2_MMUSTACK — chaine de PUSH/POP r32 consecutifs. Le registre de travail
    // laisse par le PUSH/POP precedent contient DEJA la traduction hote de la
    // pile ; l'ADD de base du suivant est alors redondant.
    // mmust_ninst : rang de la derniere instruction chainable emise (-1 = rien)
    // mmust_pos   : dyn->arm_size juste APRES son emission. L'egalite avec
    //               arm_size au moment du maillon suivant est la preuve
    //               DIRECTE que RIEN n'a ete emis entre les deux, donc que le
    //               registre de travail est intact (arm_size n'avance que par
    //               EMIT, et seulement aux passes 2 et 3).
    // mmust_kind  : 1 = PUSH r32, 2 = POP r32 (jamais melanges)
    // mmust_scr   : registre ARM de travail qui porte la traduction
    // mmust_delta : deplacement a appliquer au prochain acces (POP)
    int                 mmust_ninst;
    int                 mmust_pos;
    int                 mmust_kind;
    int                 mmust_scr;
    int                 mmust_delta;
#ifdef D2_EMITPROF
    // Index du bloc dans le registre du profil du code emis. Attribue UNE
    // fois, AVANT pass0, donc identique aux quatre passes : le prologue emis
    // (pass2 pour la taille, pass3 pour les octets) doit y trouver la MEME
    // adresse de compteur, sinon les deux passes divergent en taille et le
    // bloc est annule par le detecteur de dynarec_arm.c.
    int                 d2ep_idx;
#endif
} dynarec_arm_t;
