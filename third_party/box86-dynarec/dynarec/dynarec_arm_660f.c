...
uintptr_t dynarec660F(dynarec_arm_t* dyn, uintptr_t addr, uintptr_t ip, int ninst, int* ok, int* need_epilog)
{
    ...
    switch(opcode) {
        ...
        case 0xF7:
            ...
            GETEX(q1, 0);
            v0 = fpu_get_scratch_quad(dyn);
            // C8 (deep review): xEDI is a GUEST address — the NEON load/store
            // below must go through the fastmmu-translated copy (no-op under
            // identity, where MMU_ED selects xEDI itself).
            MMU_G2H(x1, xEDI);
            VLD1Q_8(v0, MMU_ED(x1, xEDI));  // hopefully it's always aligned?
            if(MODREG)
            ...
            VST1Q_8(v1, MMU_ED(x1, xEDI));  // put back
            break;
        case 0xF8:
